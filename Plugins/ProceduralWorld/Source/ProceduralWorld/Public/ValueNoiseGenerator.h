...
class PROCEDURALWORLD_API ValueNoiseGenerator
{
public:
	ValueNoiseGenerator();
...
template <typename T, unsigned N>
ValueNoiseGenerator<T, N>::ValueNoiseGenerator() : size{ N } {

}

template<typename T, unsigned N>
inline void ValueNoiseGenerator<T, N>::GenerateNoiseValues(unsigned seed)
{
	FMath mathInstance;
...
template<typename T, unsigned N>
inline float ValueNoiseGenerator<T, N>::processCoord(Vec2<float> coord) const
{
